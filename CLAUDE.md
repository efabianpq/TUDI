...
- Variables sensibles (API key del proveedor de IA, credenciales de MySQL) solo en `.env`, nunca hardcodeadas ni commiteadas. `.env.example` documenta cada variable esperada, incluida `AI_NUTRITION_PROVIDER_API_KEY` (comentada: todavía no hay ningún proveedor externo que la use — sección 4.9).
