...
- **Sesiones:** `SESSION_DRIVER=database`. **Nunca `file` en producción**: ese driver serializa las peticiones de una misma sesión y, con llamadas a la IA de varios segundos, dos pestañas bastan para provocar un 504 (sección 5.13). **`SESSION_LIFETIME=43200`** (30 días) y el "Mantener la sesión abierta" del login **marcado por defecto**: se entra casi siempre desde el acceso directo del móvil, donde lo que se espera de una aplicación es seguir dentro, no volver a teclear la contraseña. Con las 2 h de fábrica y la casilla vacía, cada visita del día siguiente empezaba por el login. El *remember* es lo que de verdad lo sostiene —reautentica por cookie aunque la sesión haya caducado—, así que el `lifetime` es el segundo cinturón, no el mecanismo. Quien comparta el dispositivo puede desmarcarlo. **Cambiar `SESSION_LIFETIME` en producción es un paso de `.env`**, no llega solo con un `git pull`.
...
| Objetivo sellado de cada día | `app/Services/ObjetivoDelDiaService.php` |
...
| Extras del día (fuera de las tres comidas) | `app/Services/ReporteComidaService.php` (`registrarExtra`/`eliminarExtra`/`extrasDelDia`), `app/Http/Controllers/ExtraController.php` |
...
- **`registros_diarios`**: `usuario_id` + `fecha` (único), `peso_kg` del día, las cifras del día (`calorias_objetivo_dia`, `calorias_consumidas`, `calorias_actividad_ajustada`, `deficit_diario`, y objetivo/consumido de los tres macros: `proteina_objetivo_g`/`proteina_consumida_g`, `grasa_objetivo_g`/`grasa_consumida_g`, `carbohidratos_objetivo_g`/`carbohidratos_consumidos_g`), `cerrado` + `cerrado_en`, `ingredientes_desayuno`/`ingredientes_almuerzo`/`ingredientes_cena` (texto libre por comida). Las **cuatro columnas de objetivo** se escriben al empezar el día y no al cerrarlo (sección 5.25): son el objetivo con el que se trabajó ese día, y por eso un cambio posterior de la Calculadora no las mueve. Las de **consumido** y el déficit sí las rellena el cierre.
- **`planes_comida`**: `tipo_comida` enum(desayuno,almuerzo,cena,snack), `origen` enum(plan,reporte) default `plan`, macros estimados, `descripcion` + `preparacion` + `notas_ia`, `ingredientes_detalle` (json, snapshot denormalizado a propósito — sigue siendo legible aunque se editen los ingredientes de origen). `origen = reporte` marca la fila que existe solo para colgar de ella una `ComidaReal` de algo que nunca se planificó: una comida reportada sin plan previo (sección 5.5) o un **extra** (sección 5.27, `tipo_comida = snack`). Sus macros estimados son cero porque no se sugirió nada, y al reabrir la comida —o borrar el extra— se borra entera. `snack` no tiene índice único con `registro_diario_id` a propósito: de un día puede colgar más de un extra.
...
- **Plantilla de correo con marca e idioma propios.** El tema por defecto de Laravel (`vendor:publish --tag=laravel-mail` y `--tag=laravel-notifications`, ahora en `resources/views/vendor/mail` y `resources/views/vendor/notifications`, versionados en el repo) se sobrescribió para que ningún correo transaccional salga con el morado/gris de Laravel ni con texto de framework sin traducir:
  - `resources/views/vendor/mail/html/themes/default.css` usa los mismos tokens de `resources/css/tudi-tokens.css` (fondo crema, texto carbón, botón negro tipo `tudi-btn-primary`); como es CSS de correo no puede leer variables CSS ni Tailwind, así que los valores están en hexadecimal literal y hay que mantenerlos a mano si los tokens cambian. Sin rojo (regla 8): el nivel `error` de `MailMessage` usa el ámbar de avisos.
  - `resources/views/vendor/mail/html/message.blade.php` pinta la cabecera con el isotipo (`public/icons/icon-192.png`, ya sincronizado en producción — ningún archivo nuevo que enlazar, sección 5.12) + el wordmark "tudi" en texto (no imagen, para que siga siendo legible si el cliente de correo bloquea imágenes) y traduce el pie de página.
  - `resources/views/vendor/notifications/email.blade.php` traduce el saludo/despedida de fábrica y la línea "cópiala en tu navegador" que acompaña a cada botón — antes salían en inglés porque ese texto vive en `@lang()` de un paquete que no tenía entrada en español.
  - El `ResetPassword` de Breeze (recuperar contraseña) no se tocó como vista: sus cuatro cadenas en inglés (asunto, cuerpo, expiración, aviso) ya pasan por `@lang()` en el paquete, así que se tradujeron añadiendo sus claves a `lang/es.json`, igual que cualquier otro texto de Breeze (sección 2).
  - Los cuatro `app/Notifications/*` ya escribían su propio asunto/saludo/líneas en español; lo que cambiaba era el envoltorio (cabecera, botón, pie, subcopy) que Laravel añade alrededor. Publicar y editar esas vistas es la única forma de tocar ese envoltorio sin parchear `vendor/`.
...
- **Un cambio aquí alcanza al día de hoy y a ninguno anterior** (sección 5.25): cada día guarda el objetivo con el que se trabajó, así que los días pasados —incluso los que siguen abiertos— conservan el suyo. Al guardar se vuelve a sellar el día en curso, que es para el que el usuario acaba de pedir el cambio.
...
- **`OpenAiMealDistributionProvider`** (vigente): `chat/completions` con salida estructurada **estricta** (`response_format.json_schema`, `strict: true`), `temperature = 0.1`, `max_completion_tokens`, `OPENAI_MODEL=gpt-4.1`. Timeout 20 s / connect 5 s (sección 5.13). Ningún fallo produce 500: `MealDistributionUnavailableException` cubre sin clave, fallo del proveedor, negativa explícita (`refusal`), filtro de contenido, corte por longitud, respuesta ininterpretable, cuota agotada o "nada que distribuir". El texto del usuario se guarda aunque la generación falle. `GeminiMealDistributionProvider` y `ClaudeMealDistributionProvider` siguen en el repo sin bindear (sección 6).
  - **El modelo da la composición por 100 g; las calorías de la porción las multiplica PHP.** El esquema separa dos cosas que antes venían fundidas en un solo campo "calorías de la porción": la **composición por 100 g**, que es un dato de tabla del alimento y no depende de nada del día, y los **gramos asignados**, que son la decisión de reparto. Fundidas, el modelo tenía dos variables libres para cuadrar el objetivo de la comida y movía la que no debía — se vio una sopa de frijol a 48 kcal/100 g, la mitad de cualquier tabla, servida en una ración de 400 g, de forma que el total caía clavado en el presupuesto del almuerzo: la persona leía una ración generosa y un déficit que no era el suyo. Con la densidad como entrada, la única palanca que le queda son los gramos, que es justo la que debe moverse: *"400 g de esta sopa son 380 kcal, te dejo 200 g"* es el consejo que se vino a buscar.
    - **El campo `calorias` ya no existe en el esquema.** Una instrucción de prompt se puede ignorar; un campo ausente no se puede rellenar con el número que conviene.
    - **El orden de los campos tampoco es cosmético:** la salida estructurada se genera campo a campo, así que poniendo la densidad antes de `cantidad_g` el modelo queda comprometido con el dato antes de saber cuántos gramos le caben en el presupuesto.
    - **`porcion` la compone PHP** a partir de `cantidad_g` + `unidad` + `detalle`, para que lo que se lee y lo que se calcula no puedan divergir (antes eran dos campos independientes que podían contradecirse).
    - **`densidadVerificada()`** contrasta cada densidad contra sus propios macros (Atwater) y ante una contradicción manda la suma de los macros. Caza la respuesta **incoherente**, no el alimento subestimado de forma coherente; contra eso actúan el orden de los campos y el prompt. **La comprobación es asimétrica**, y no es un detalle: solo corrige cuando Atwater queda *por encima* de las kcal declaradas más de un 30 %, que es físicamente imposible —los macros ya suman más energía de la que el alimento dice tener— y es la forma exacta del error de la sopa. Por debajo no se toca nunca: el alcohol aporta 7 kcal/g y no es proteína, ni grasa, ni carbohidrato, así que una cerveza son ~43 kcal/100 g con macros que suman 16 sin nada de incoherente. Corregir también hacia abajo dejaría media carta de bebidas contada a un tercio, justo en la categoría que más falta hace registrar bien (sección 5.27).
    - Las densidades se conservan en `ingredientes_detalle.por_100g`: son el dato auditable ante una cifra rara —dice si falló la tabla o la ración— y la semilla de una futura tabla local de alimentos, que es lo que haría el cálculo determinista y dejaría al modelo solo emparejando texto con alimento.
  - **Tablas colombianas, no solo USDA.** El prompt manda al ICBF para alimentos, platos y cortes locales: "lomo ancho" es un corte de res con su grasa (~225 kcal/100 g), no el solomillo magro que sale de leer ese nombre contra USDA.
  - **Al estimar lo que ya se comió no viaja ningún objetivo.** `promptDeConsumoReal()` no incluye el objetivo del día aunque el contexto lo traiga: ahí no hay nada que cuadrar, así que esa cifra no podía hacer otra cosa que funcionar de ancla. Lo comido, comido está.
...
**Un solo botón que cambia de papel.** "Cerrar desayuno" abre los campos del reporte dentro de la tarjeta y se convierte en "Confirmar cierre"; con la comida ya cerrada queda "Reabrir desayuno". Así la tarjeta no enseña un formulario de reporte a quien todavía no ha comido. Visualmente es del mismo tamaño que "Rehacer solo el X" (`tudi-btn-secondary`, transparente con borde, ancho completo en móvil, ancho de su texto en escritorio): una acción normal de la tarjeta, nunca al nivel de "Calcular mi plan" (`tudi-btn-lime`, sección 5.12), que es la que reparte el día entero.
...
- **"Copiar lo sugerido y ajustar cantidades"** es el tercer atajo de escritura, y resuelve el caso más común de todos: se cumplió el plan *casi*, pero no al gramo. El botón escribe en el campo de texto los ingredientes del `PlanComida` con su porción (`nombre` + `porcion`, separados por comas; cae a `descripcion` si el plan no trae detalle) y ahí se corrigen las cantidades. Como los demás chips, **solo escribe: no envía el formulario ni llama a nadie**. Solo aparece si la comida tiene plan previo. A diferencia de "lo que sueles comer", este camino **sí gasta cuota** en cuanto se envía, porque el texto sale editado y esos macros no están calculados en ninguna parte — el atajo gratuito para un cumplimiento exacto sigue siendo el interruptor. Existe porque obligar a reescribir la comida entera para corregir una cantidad es justo lo que hace que se deje de reportar, y un hueco sin reportar entra luego en el promedio móvil como calorías que nunca se comieron (sección 5.24).
...
- **2. Hoy** — tres sub-estados sobre el `RegistroDiario` de hoy, más el caso transversal de parámetros incompletos (mismo mensaje que antes, con CTA a la Calculadora):
  ...
  - *En curso* — **el anillo deja de ser la cifra y pasa a ser el indicador**: `.tudi-ring-mini` (76px) con su porcentaje dentro (`--pct` = consumidas / (objetivo + actividad), acotado 0–100), y al lado la cifra protagonista, que es la accionable: **lo que queda** (`MealDistributionService::saldoDelDia()`, sección 5.21: "Te quedan / Te pasaste por X kcal para [comidas pendientes]"), nunca un "déficit" — ese sustantivo de resultado solo se gana con el día cerrado (regla transversal más abajo). Completan el panel **los tres macros** —proteína, grasas y carbohidratos, cada uno real contra objetivo con su barra (lima, ámbar y crema)—, las tres comidas como **chips en línea** (lima con ✓ la cerrada, blanca la planificada, apagada la pendiente) y el enlace al plan. Los tres macros y no solo la proteína: el objetivo del día son cuatro cifras y enseñar una sola dejaba el panel contando media historia. Todo dentro del único panel carbón de la pantalla: en móvil apilado, en `sm:` dos mitades.
  - *Cerrado* — **la misma tarjeta, no otra**: mismo panel carbón, mismo anillo, mismos macros, mismas comidas. Solo cambian sus cifras, que ahora salen del snapshot congelado, y el rótulo de la grande, que pasa a "Déficit de N kcal · resultado del día" — con el día cerrado el déficit ya es un resultado y por fin se llama por su nombre. Añade un chip "cerrado" y el CTA pasa a "Ver el detalle del día". Sustituirla por un resumen distinto obligaba a reaprender la pantalla justo al terminar el día; la tarjeta "Resultado real del día" (`x-tudi.resultado-dia`) sigue siendo la del **plan diario**, no la de Inicio.
...
- **El tramo se reconstruye entero, no se va sumando.** El manejador de `result` recorre `evento.results` **desde cero** y rehace la cadena, en vez de añadir lo nuevo desde `evento.resultIndex`. Chrome en Android reenvía resultados que ya había dado por definitivos con un `resultIndex` que no siempre avanza, y sumarlos otra vez repetía las palabras — se leía como "el micrófono entiende mal" cuando era el mismo texto concatenado dos y tres veces. Rehacer la cadena completa es idempotente, y `results` es por especificación la transcripción entera de la sesión. Con reenganche (iOS), lo reconocido por el tramo pasa a `definitivo` **antes** de abrir el siguiente, que numera sus resultados desde cero.
- **Un dictado a la vez.** `popup.abierto` hace de cerrojo y el botón del micrófono ignora los toques mientras haya uno en curso: dos toques seguidos arrancaban dos reconocedores sobre el mismo audio, y cada uno escribía su versión en el campo.
- **Vigilante de silencio.** En algunos Android, `start()` no lanza ningún error pero el micrófono nunca llega a abrirse de verdad: ni `result` ni `error`, así que sin nada más el popup se quedaba abierto para siempre y el botón parecía no hacer nada. `TIEMPO_ESPERA_AUDIO_MS` (4 s) espera el evento `audiostart`; si no llega, aborta la sesión, cierra el popup y avisa que hay que revisar el permiso del micrófono o escribir a mano — el mismo mensaje que un `error` real.
...
- **`resources/css/tudi-tokens.css`** es la única fuente de verdad de color, tipografía, radio y espaciado; se importa antes de las directivas de Tailwind. `tailwind.config.js` refleja los mismos valores — si un token cambia, se cambia en el CSS y se copia allí, nunca al revés. Fondo crema siempre, un panel carbón por pantalla con la cifra protagonista, **lima solo para progreso** (sobre crema el primario es carbón), ámbar para avisos (no hay rojo en la paleta). **Excepción acordada, pedida explícitamente por el usuario: los botones que avanzan el día van en lima, no en carbón.** `planes.crear` (Inicio, bienvenida y el listado de Planes) y "Calcular mi plan" (el plan diario) son `tudi-btn-lime`. El texto queda carbón sobre lima, que es la combinación con contraste. **Sobre crema ese botón lleva contorno**: `.tudi-btn-lime` nació para los paneles carbón, donde el lima brillante recorta solo contra el oscuro, pero sobre crema los dos son claros y cálidos, el canto se deshacía y el botón dejaba de leerse como una pieza. Lleva `1px` de borde en `--tudi-lime-700` y una sombra de `1px` del mismo color, que desaparece al pulsar mientras el botón baja ese píxel. Ningún color nuevo: los dos salen de los tokens. Dentro de `.tudi-panel` o `.on-dark` el contorno se retira, porque ahí sobra. La regla "lima solo para progreso" sigue rigiendo para todo lo demás — esto no la deroga, es la única acción de la aplicación que se trató como excepción, y ambos botones cubren la misma clase de gesto: la acción principal que hace avanzar el día.
...
- **Navegación:** barra lateral de 232px en escritorio; en móvil, **dos barras fijas** — la superior (`x-tudi.barra-superior`: marca, fecha y menú de la cuenta) y la inferior de tres destinos (Inicio/Calculadora/Planes), con `aria-current="page"` en el activo. Las dos viven en el layout, no en cada vista: antes cada pantalla montaba su cabecera y solo algunas incluían el menú del avatar, así que desde el plan diario no había forma de llegar a "Mi cuenta" ni de cerrar sesión. En escritorio la barra superior se oculta porque la lateral ya trae marca, tarjeta de usuario y logout. Objetivos táctiles de 44px mínimo. Componentes compartidos (`x-text-input`, `x-primary-button`) fijan 48px de alto y 16px de tipografía (evita el zoom automático de Safari).
  - **La barra inferior se retira mientras se escribe** (`resources/js/tudi/barra-inferior.js` + `.tudi-escribiendo`). Es `position: fixed`, o sea anclada al viewport de maquetación, y el teclado del sistema lo encoge: la barra subía con él y se quedaba flotando a media pantalla — se notaba sobre todo en el plan diario, la única pantalla llena de campos. `interactive-widget` no lo arregla (`resizes-visual` la deja fija pero detrás del teclado; `resizes-content` es justo el comportamiento que la sube), así que la única salida honesta es que no esté: mientras se escribe, tres destinos de navegación no aportan nada y sí roban alto útil. Se detecta por `focusin`/`focusout` sobre campos de texto —no por la altura del `visualViewport`— para cubrir también el teclado físico y los campos que aparecen después; los checkbox y radio quedan fuera porque no abren teclado.
- **Sin rebote de página** (`overscroll-behavior-y: none` en `html, body`, `tudi-tokens.css`). En una pantalla que no llega a llenar el alto —el listado de Planes con pocos días, por ejemplo— arrastrar un poco de más hacía rebotar la página entera: el rebote mueve el viewport visual, así que aunque `.tudi-topbar`/`.tudi-tabbar` sean `fixed`, se veían despegarse de golpe y dejaban un hueco del color de fondo hasta que la animación de vuelta terminaba. En una pantalla más larga el mismo gesto se leía como scroll normal y no se notaba nunca — por eso el hueco solo aparecía en pantallas cortas, en iOS y Android por igual.
- **Safe area, arriba y abajo.** Con `viewport-fit=cover` + `apple-mobile-web-app-status-bar-style: black-translucent`, instalada como app en iOS la página empieza **debajo del reloj y la señal**. `main` descuenta `env(safe-area-inset-top)` en móvil (`pt-[calc(env(safe-area-inset-top)+3.75rem)]`, `sm:pt-8` — el hueco creció respecto de su valor original para hacer sitio a la barra superior fija, `x-tudi.barra-superior`) y el layout de invitado hace lo mismo arriba y abajo; sin eso, la cabecera —y con ella el menú de la cuenta— quedaba solapada con la barra del sistema y no se podía pulsar. En el navegador, sin instalar, el inset es cero y el espaciado es el de siempre.
...
- **Acordeón de comidas** en el plan diario: `<details>` nativos con un solo abierto a la vez (listener en captura, `toggle` no burbujea). **Abrir una comida no mueve la pantalla:** al cerrar las demás desaparece de golpe el alto de las que quedaban por encima y, como el scroll no se ha movido, la tarjeta recién abierta se iba fuera de la pantalla y había que volver a buscarla. `sinSaltoDeScroll()` (`resources/js/tudi/formularios.js`) ancla la tarjeta pulsada a su posición en el viewport, ejecuta el cierre de las otras y corrige el scroll por la diferencia. Cualquier acción nueva que cambie el alto por encima de lo que se está mirando debe pasar por ahí.
...
- **El contador vive en la caché**, con clave por usuario, concepto y fecha local, y expira solo a medianoche. No hace falta tabla: no es un dato del dominio, no se consulta históricamente y perderlo solo regala el resto del día.
- **El contador solo se enseña cerca del límite, no desde la primera pulsación del día.** `CuotaIaService::cercaDelLimite()` es cierto a partir de `UMBRAL_AVISO` (80 %) del límite consumido: con los valores de fábrica, desde la décima distribución (de 12) y desde el duodécimo reporte (de 15). Antes se pintaba siempre, y "te quedan 15 de 15 hoy" es ruido puro — no cambia ninguna decisión, porque nadie raciona algo que le sobra, y compite por la atención con lo único que importa en esa pantalla, que es registrar la comida (regla 8). El aviso se gana su sitio cuando el límite está cerca de verdad y sí puede cambiar lo que la persona hace a continuación, y **con la cuota agotada se dice siempre**: ahí deja de ser un contador y pasa a explicar qué se puede hacer igualmente. **Quién decide es el servicio, no la vista** (mismo criterio que `DashboardEstadoService`, sección 5.8): el controlador pasa `avisarCuotaDistribucion`/`avisarCuotaReporte` ya resueltos.
...
`DashboardEstadoService::comidasSinReportarAyer()` (privado, alimenta la clave `avisoAyer` de `calcular()` — `DashboardController` solo compone, sección 5.8): si el plan de **ayer** existe, tiene alguna comida reportada y alguna sin reportar, Inicio muestra una línea en ámbar con las que faltan y un enlace al día para completarlo (reabriéndolo si hiciera falta).
...
### 5.25 El objetivo queda sellado en cada día

`ObjetivoDelDiaService`. **Cada `RegistroDiario` guarda el objetivo con el que se trabajó ESE día**, y cambiar la Calculadora Déficit no lo reescribe hacia atrás.

Antes, cualquier día abierto derivaba sus objetivos del perfil actual: tocar la Calculadora reescribía los de todos los días pasados que siguieran abiertos, y un día de la semana pasada —planificado, reportado y cerrado a medias contra 2.112 kcal— pasaba a compararse contra 1.848 kcal sin que en él hubiera cambiado nada. El historial de la ventana de 7 días acababa medido con una regla distinta de la que se usó para vivirlo.

- **Dónde se guarda:** `calorias_objetivo_dia`, `proteina_objetivo_g`, `grasa_objetivo_g` y `carbohidratos_objetivo_g`, que ya existían. Lo que cambia es **cuándo** se escriben: antes solo al cerrar, ahora al empezar el día. El significado es el mismo —el objetivo vigente de ese día—, solo que se conoce desde el principio. **No hay columnas ni tablas nuevas.**
- **Quién sella:** `sellar()` al crear el día (`planes.crear`, `IngredienteDisponibleController@store`, `DemoSeeder`) y al reiniciarlo (`PlanDiarioService::resetear()` — "como recién creado" incluye el sello). `sellarElDiaDeHoy()` tras un cambio de la Calculadora (`ProfileParametersController@update`) y tras confirmar un ajuste (`RulesEngineService::confirmar()`).
- **El día de hoy es la excepción deliberada:** un cambio de la Calculadora sí alcanza al día en curso, porque es para el que el usuario acaba de pedirlo. A ningún día anterior.
- **Un día cerrado no se re-sella nunca**, ni siendo el de hoy: sus cifras están congeladas y reescribir su objetivo movería un déficit ya consolidado.
...
- `ObjetivoDelDiaService::calcularDelUsuario(User)` sigue existiendo para cuando hay usuario pero todavía no hay día (la sugerencia de actividad, la Calculadora); `MealDistributionService::objetivosDelDia(User, ?array $reparto)` es quien lo envuelve para ese caso.
...
### 5.27 Extras: lo que se come fuera de las tres comidas

Una gaseosa, una cerveza, un postre a media tarde, unas onces, galletas. Se registran, **nunca se planifican**, y esa distinción es el diseño entero.

**Un extra no recibe presupuesto: gasta el saldo.** La tentación obvia era meter `snack` en `DISTRIBUCION_COMIDAS` con un 10 % del día, y sería un error en dos direcciones: convertiría a TUDI en cómplice —"tienes 200 kcal de cerveza asignadas hoy"— y dejaría cortas a las tres comidas los días en que no se pica nada. Lo que hace es entrar en `saldoDelDia()` (sección 5.21): se registra, el saldo baja y "Calcular mi plan" reparte menos entre las comidas que faltan. *"Te quedan 980 kcal para la cena"* → registras la cerveza → *"Te quedan 830"*. La lección la da la aritmética, sin sermón ni bloqueo, y si el día se pasa ya estaba resuelto: saldo negativo en ámbar con "te pasaste por X kcal".

**Cero tablas y cero columnas nuevas.** `tipo_comida = 'snack'` estaba en el enum de `planes_comida` desde la primera migración sin usarse, y `origen = 'reporte'` existe justamente para un `PlanComida` que solo sostiene una `ComidaReal` de algo que nunca se planificó (sección 4) — eso **es** un extra. No hay índice único sobre `(registro_diario_id, tipo_comida)`, así que varios extras al día ya cabían.

- **`DISTRIBUCION_COMIDAS` no cambia, y es deliberado.** Esa constante declara quién recibe parte del reparto. Al quedarse fuera, los chips de Inicio, el checklist de diagnóstico y `comidasSinReportar()` siguen hablando de tres comidas sin tocar una línea: un extra sin registrar no es un hueco, porque no había nada que reportar.
- **Ruta propia** (`extras.store`/`extras.destroy`, `ExtraController`), no una reutilización de `comidas.cerrar`: un extra no tiene plan que cerrar y se añade N veces al día, mientras que una comida se cierra una vez y se reabre. Meter los dos casos por el mismo endpoint habría sido un `if` por cada diferencia.
- **Borrar un extra** reusa `ComidaRealService::eliminar()` tal cual: se lleva la imagen, la `ComidaReal` y **el propio `PlanComida`** por ser `origen = reporte`, y recalcula las calorías del día. Como reabrir una comida, **no devuelve cuota** (sección 5.20): si la devolviera, añadir y borrar el mismo extra sería una llamada gratis infinita.
- **Los frecuentes valen igual** (sección 5.22). "Son muchos alimentos" es cierto del catálogo, no de una persona: nadie toma una cerveza distinta cada tarde. La primera vez cuesta una llamada; de la segunda en adelante es un chip de un toque y gratis.
- **Sin plan de referencia en el prompt.** `estimarConsumoReal()` acepta ahora `plan` opcional. Antes se mandaba siempre, y sin plan previo salía *"sin descripción — 0 kcal, P 0 g, G 0 g, C 0 g"*: eso no le dice al modelo "no había plan", le dice "se le sugirió no comer nada" — una referencia falsa que además arrastra la estimación hacia abajo. Un extra nunca tiene plan, por definición.
- **Bug que esto destapó y se arregló de paso:** `saldoDelDia()` recorría `DISTRIBUCION_COMIDAS` sobre un `keyBy('tipo_comida')`, así que no habría contado ningún extra y, con varios, solo habría sobrevivido el último. El cierre del día, en cambio, siempre sumó **todas** las `ComidaReal` sin mirar el tipo: el saldo en vivo y el cierre podían decir cosas distintas del mismo día. Ahora los dos usan ese mismo criterio, y los pendientes se siguen sacando solo de las tres comidas.
- **En pantalla:** cuarta tarjeta del acordeón, después de las tres comidas, con la lista de lo registrado, su kcal y una `X` para quitarlo, más el campo de texto (con dictado) y los chips de frecuentes. La cabecera resume `Extras · 320 kcal` sin abrirla, y vacía dice "nada aún". **No tiene plan, ni "Rehacer", ni porcentaje**, porque no es una cuarta comida. Nunca arranca abierta: se ofrece, no se reclama. Discreta pero visible — escondida, nadie registra la cerveza y el déficit miente; como panel grande con estado vacío, invita a llenarla.
...
7. **Ninguna cifra que entre al balance energético la calcula un modelo de IA.** El modelo aporta **datos de tabla** (la composición por 100 g de un alimento) y decisiones de reparto en **gramos**; todas las multiplicaciones, sumas, objetivos y déficit los calcula PHP con `NutritionCalculatorService`. Si una cifra del balance se puede escribir directamente en un campo de la respuesta del modelo, ese campo sobra: quítalo del esquema en vez de pedir por prompt que no se use (sección 5.3).
